    ...
    view! {
        ...
        >
            ...
            <div class="space-y-10">
                ...
                <ChartCard title="Taproot Spend Types" description=chart_desc(range, "Key-path vs script-path spends per block. How Taproot is actually being used", "Daily average key-path vs script-path spends. How Taproot is actually being used") chart_id="chart-taproot-spend-types" option=taproot_spend_type_option info="Key-path spends look like regular single-sig transactions on-chain, revealing no script details. Script-path spends reveal that a more complex script was involved (multisig, timelocks, etc.). A high key-path ratio suggests most Taproot usage is for simple payments rather than complex contracts."/>
                ...
                <ChartCard title="P2PKH Sunset Tracker" description="Decline of legacy P2PKH address usage over time. Horizontal lines mark 10% and 5% thresholds" chart_id="chart-p2pkh-sunset" option=sunset_option info="Tracks P2PKH (legacy '1' addresses) as a share of total outputs. The 90-day moving average smooths out noise. Horizontal lines mark the 10% and 5% thresholds. Crossing below these levels indicates the ecosystem is shifting away from legacy address formats."/>

                // ── Transactions ─────────────────────────
                <SectionHeading id="section-tx-metrics" title="Transactions"/>
                <ChartCard title="RBF Adoption" description=chart_desc(range, "Percentage of transactions opting into Replace-By-Fee per block", "Daily average RBF adoption percentage") chart_id="chart-rbf" option=rbf_option info="Replace-By-Fee (BIP 125) lets senders bump fees on unconfirmed transactions. A transaction signals RBF by setting at least one input's sequence number below 0xfffffffe. Higher adoption means more wallets support fee bumping, which can help users during congestion."/>
