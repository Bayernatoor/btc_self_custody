...
fn back_button_view(label: &'static str, on_click: impl Fn(leptos::ev::MouseEvent) + 'static) -> impl IntoView {
    view! {
        <button
...
pub fn GuideSelector() -> impl IntoView {
    ...
    let (selected_platform, set_selected_platform) = signal(None::<&'static str>);

    // Step: 1 = level, 2 = platform, 3 = OS (desktop only)
    let step = Signal::derive(move || {
        if selected_platform.get().is_some() { 3u8 }
        else if selected_level.get().is_some() { 2u8 }
        else { 1u8 }
    });
    let total_steps = Signal::derive(move || {
        // 3 steps if a level with desktop is selected, otherwise 2
        if selected_platform.get() == Some("desktop") { 3u8 }
        else if selected_level.get().is_some() {
            let level = selected_level.get().and_then(guides::find_level);
            if level.map_or(false, |l| l.platforms.contains(&"desktop")) { 3u8 } else { 2u8 }
        } else { 2u8 }
    });
