...
pub fn NetworkChartsPage() -> impl IntoView {
    ...
    let address_type_pct_option = chart_signal!(dashboard_data, range, overlay_flags,
        |blocks| crate::stats::charts::address_type_pct_chart(blocks),
        |days| crate::stats::charts::address_type_pct_chart_daily(days)
    );

    let taproot_spend_type_option = chart_signal!(dashboard_data, range, overlay_flags,
    ...
    let batching_option = chart_signal!(dashboard_data, range, overlay_flags,
