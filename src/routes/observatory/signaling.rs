                    ...
                    match result {
                        Ok(((ref blocks, ref period_stats), ref periods, p_start, p_end, _current_p, _target_p)) => {
                            ...
                            let bip_desc: (&str, &str, &str, &str) = if bip_method.get() == "locktime" {
                                ("BIP-54: Consensus Cleanup", "Addresses four protocol vulnerabilities: timewarp attack, worst-case block validation time, 64-byte transaction exploits, and a theoretical edge case in BIP-34's duplicate coinbase txid prevention. There is no formal signaling mechanism for BIP-54. This tracker checks compliance: coinbase nLockTime = height\u{2009}\u{2212}\u{2009}1 and nSequence != 0xFFFFFFFF (timelock not disabled).", "95%", "https://github.com/bitcoin/bips/blob/master/bip-0054.md")
                            } else {
