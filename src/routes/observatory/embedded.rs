                ...
                Some(Ok(_)) => {
                    ...
                    view! {
                        <div class="space-y-10">
                            ...
                            <ChartCard title="All Embedded Data — Block Share" description=chart_desc(range, "How much of each block is non-financial data (OP_RETURN outputs plus witness inscriptions)", "Daily average non-financial data share per block") chart_id="chart-all-embedded-share" option=all_embedded_share_option info="Combines OP_RETURN data (in outputs) and inscription data (in witness) as a percentage of total block size. These are disjoint categories. A rising share means more block space is being used for non-financial purposes."/>
