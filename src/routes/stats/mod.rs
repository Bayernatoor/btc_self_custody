    ...
    view! {
        ...
        <section class="max-w-[1600px] mx-auto px-4 lg:px-10 pt-10 pb-28 opacity-0 animate-fadeinone">
            ...
            <div class=move || if tab.get() == "opreturn" { "block" } else { "hidden" }>
                ...
                // Protocol timeline and info cards
                {
                    let (active_protocol, set_active_protocol) = signal(String::new());
                    view! {
                        // Timeline bar
                        <div class="bg-[#0d2137] border border-white/10 rounded-2xl p-4 lg:p-5 mb-6">
                            <div class="flex items-center justify-between mb-3">
                                <h3 class="text-sm text-white/50 uppercase tracking-widest font-semibold">"Protocol Timeline"</h3>
                                <span class="text-xs text-white/30">"Click a protocol for details"</span>
                            </div>
                            // Timeline visualization
                            <div class="relative h-24 mb-2">
                                // Year markers
                                <div class="absolute inset-x-0 bottom-0 flex justify-between text-[0.6rem] text-white/20 px-1">
                                    {["2012", "2014", "2016", "2018", "2020", "2022", "2024", "2026"].into_iter().map(|y| {
                                        view! { <span>{y}</span> }
                                    }).collect::<Vec<_>>()}
                                </div>
                                // Baseline
                                <div class="absolute inset-x-0 bottom-4 h-px bg-white/10"></div>
                                // Protocol bars — positioned by year (2012=0%, 2026=100%)
                                // Omni Layer: 2013-present
                                <button class="absolute h-4 rounded-full cursor-pointer transition-all hover:brightness-125 border border-transparent hover:border-white/30"
                                    style="left: 7%; right: 0%; bottom: 16px; background: #3b82f6"
                                    title="Omni Layer (2013–present)"
                                    on:click=move |_| set_active_protocol.update(|p| { if *p == "omni" { p.clear() } else { *p = "omni".into() }})
                                ></button>
                                // Counterparty: 2014-present
                                <button class="absolute h-4 rounded-full cursor-pointer transition-all hover:brightness-125 border border-transparent hover:border-white/30"
                                    style="left: 14%; right: 0%; bottom: 34px; background: #f59e0b"
                                    title="Counterparty (2014–present)"
                                    on:click=move |_| set_active_protocol.update(|p| { if *p == "counterparty" { p.clear() } else { *p = "counterparty".into() }})
                                ></button>
                                // Stamps: 2023-present
                                <button class="absolute h-4 rounded-full cursor-pointer transition-all hover:brightness-125 border border-transparent hover:border-white/30"
                                    style="left: 78.5%; right: 0%; bottom: 52px; background: #94a3b8"
                                    title="Stamps/SRC-20 (2023–present)"
                                    on:click=move |_| set_active_protocol.update(|p| { if *p == "stamps" { p.clear() } else { *p = "stamps".into() }})
                                ></button>
                                // Ordinals: Jan 2023-present
                                <button class="absolute h-4 rounded-full cursor-pointer transition-all hover:brightness-125 border border-transparent hover:border-white/30"
                                    style="left: 78.5%; right: 0%; bottom: 16px; background: #ec4899"
                                    title="Ordinals (2023–present)"
                                    on:click=move |_| set_active_protocol.update(|p| { if *p == "ordinals" { p.clear() } else { *p = "ordinals".into() }})
                                ></button>
                                // BRC-20: Mar 2023-present
                                <button class="absolute h-4 rounded-full cursor-pointer transition-all hover:brightness-125 border border-transparent hover:border-white/30"
                                    style="left: 80%; right: 0%; bottom: 70px; background: #f472b6"
                                    title="BRC-20 (2023–present)"
                                    on:click=move |_| set_active_protocol.update(|p| { if *p == "brc20" { p.clear() } else { *p = "brc20".into() }})
                                ></button>
                                // Runes: Apr 2024-present
                                <button class="absolute h-4 rounded-full cursor-pointer transition-all hover:brightness-125 border border-transparent hover:border-white/30"
                                    style="left: 85.7%; right: 0%; bottom: 34px; background: #ff6b6b"
                                    title="Runes (2024–present)"
                                    on:click=move |_| set_active_protocol.update(|p| { if *p == "runes" { p.clear() } else { *p = "runes".into() }})
                                ></button>
                                // Labels on bars
                                <span class="absolute text-[0.55rem] text-white/70 font-medium pointer-events-none" style="left: 8%; bottom: 18px">"Omni"</span>
                                <span class="absolute text-[0.55rem] text-white/70 font-medium pointer-events-none" style="left: 15%; bottom: 36px">"Counterparty"</span>
                                <span class="absolute text-[0.55rem] text-white/70 font-medium pointer-events-none" style="left: 79.5%; bottom: 54px">"Stamps"</span>
                                <span class="absolute text-[0.55rem] text-white/70 font-medium pointer-events-none" style="left: 79.5%; bottom: 18px">"Ordinals"</span>
                                <span class="absolute text-[0.55rem] text-white/70 font-medium pointer-events-none" style="left: 81%; bottom: 72px">"BRC-20"</span>
                                <span class="absolute text-[0.55rem] text-white/70 font-medium pointer-events-none" style="left: 86.7%; bottom: 36px">"Runes"</span>
                            </div>
                        </div>

                        // Expandable info card
                        <Show when=move || !active_protocol.get().is_empty()>
                            <div class="bg-[#0a1a2e] border border-white/10 rounded-xl p-5 mb-6 animate-fadeinone">
                                {move || {
                                    let p = active_protocol.get();
                                    let (name, year, method, method_color, desc, fact) = match p.as_str() {
                                        "omni" => ("Omni Layer", "2013", "OP_RETURN", "#3b82f6",
                                            "Originally Mastercoin — the first altcoin-on-Bitcoin protocol. Tether (USDT) launched here in 2014 before migrating to Ethereum. Uses the 'omni' magic prefix in OP_RETURN outputs.",
                                            "At its peak in 2018, Omni transactions accounted for ~50% of all OP_RETURN outputs on Bitcoin."),
                                        "counterparty" => ("Counterparty (XCP)", "2014", "OP_RETURN", "#f59e0b",
                                            "Created XCP tokens by provably burning ~2,100 BTC. Enables token issuance, decentralized exchange, and betting on Bitcoin. Uses 'CNTRPRTY' prefix in OP_RETURN.",
                                            "The ~2,100 BTC burned to create XCP would be worth over $150M today — one of Bitcoin's most expensive experiments."),
                                        "stamps" => ("Stamps / SRC-20", "2023", "Bare Multisig", "#94a3b8",
                                            "Embeds image data directly in bare multisig output scripts. Unlike Ordinals, this data is stored in the UTXO set and cannot be pruned — it persists on every full node forever.",
                                            "Stamps was deliberately designed to be unprunable as a response to discussions about pruning Ordinals witness data."),
                                        "ordinals" => ("Ordinals", "Jan 2023", "Witness Data", "#ec4899",
                                            "Created by Casey Rodarmor. Assigns a unique serial number to every satoshi ('ordinal theory') and enables inscribing arbitrary data (images, text, HTML) into the witness field of transactions. Uses the SegWit discount — inscription data costs 1/4 the fee rate of regular data.",
                                            "The first inscription (block 774,628) was a pixel art skull. Within months, over 1 million inscriptions were created."),
                                        "brc20" => ("BRC-20 Tokens", "Mar 2023", "Witness Data (JSON)", "#f472b6",
                                            "Fungible token standard built on Ordinals inscriptions. Uses JSON payloads with deploy/mint/transfer operations. Caused the largest fee spike in Bitcoin history in May 2023 when the mempool exceeded 500,000 pending transactions.",
                                            "BRC-20 tokens are entirely off-protocol — Bitcoin nodes don't validate them. An off-chain indexer interprets the JSON inscriptions to determine token balances."),
                                        "runes" => ("Runes", "Apr 2024", "OP_RETURN", "#ff6b6b",
                                            "Also by Casey Rodarmor (Ordinals creator). A fungible token protocol using OP_RETURN with the OP_13 prefix. Designed as a cleaner, more efficient alternative to BRC-20. Launched at block 840,000 — the same block as Bitcoin's 4th halving.",
                                            "Runes launched at the exact halving block, causing record-high fees as users rushed to etch the first tokens. Fees exceeded 1,000+ sat/vB."),
                                        _ => ("", "", "", "", "", ""),
                                    };
                                    view! {
                                        <div class="flex items-start gap-4">
                                            <div class="w-1 h-16 rounded-full shrink-0" style=format!("background: {method_color}")></div>
                                            <div class="flex-1">
                                                <div class="flex items-baseline gap-3 mb-1">
                                                    <h4 class="text-lg text-white font-semibold">{name}</h4>
                                                    <span class="text-xs text-white/30 font-mono">{year}</span>
                                                    <span class="text-xs px-2 py-0.5 rounded-full border border-white/10 text-white/50">{method}</span>
                                                </div>
                                                <p class="text-sm text-white/60 leading-relaxed mb-2">{desc}</p>
                                                <p class="text-xs text-[#f7931a]/70 italic">"\u{1f4a1} "{fact}</p>
                                            </div>
                                        </div>
                                    }
                                }}
