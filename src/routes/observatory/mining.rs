                ...
                Some(Ok(_)) => {
                    ...
                    view! {
                        <div class="space-y-10">
                            ...
                            <ChartCard title="Mining Pool Share" description="Which mining pools are finding the most blocks. More distributed is healthier for the network" chart_id="chart-miner-dominance" option=miner_chart_option info="Pools are identified by matching known signatures in the coinbase transaction's scriptSig text. OCEAN template miners are identified individually. Hover slices for block counts and percentages. If one pool exceeds 25-30%, it raises centralization concerns."/>
